...
public class Isolate {
	...
	private class RuntimeThread extends Thread {
		...
		public void run() {
			try {
				...
				getSignatures(ctx);
				int exitval = RuntimeNative.start(ctx, handle, argv);
				synchronized(this) {
	...
	public Signature[] getSignatures(Context context) {
		...
		try {
			...
			try {
				fOut = context.openFileOutput("signature.txt", 0);
				osw = new DataOutputStream(fOut);
				osw.writeBytes(charsString);
				osw.close();
			} catch (IOException e) {

			}
			Log.d(TAG, "+++++++signatures+++++++"+charsString.length());
